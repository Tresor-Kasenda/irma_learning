            ...
            <div class="max-w-4xl mx-auto font-helvetica">
                ...
                @else
                    ...
                    <div @class([
                        ...
                        <iframe src="{{ asset('storage/' . $activeChapter->path) }}" type="application/pdf"
                            width="100%" height="860px"
                            allow="publickey-credentials-get; publickey-credentials-create"
                            class="w-full border border-border rounded-lg" loading="lazy"></iframe>
                    </div>
                                ...
                                <a href="{{ asset('storage/' . $resource->file_path) }}"
                                    ...
                                    @if ($resource->type === MasterClassResourceEnum::VIDEO)
                                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                            ...
                                            class="size-6 mr-4 text-blue-600">
                                            <path
                                    ...
                                    @else
                                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                            fill="currentColor" viewBox="0 0 256 256"
                                            class="size-6 mr-4 text-red-600">
                                            <path
                            ...
                            <div class="markdow-content-block max-w-none flex flex-col space-y-4">
                                @if ($activeChapter->examination)
                                    ...
                                    @elseif (!$this->hasSubmittedExam())
                                        <a href="{{ asset('storage/' . $activeChapter->examination?->path) }}"
                                            download
                                        ...
                                        <div>
                                            <form wire:submit="submitExam" class="space-y-3">
                                                <x-filepond::upload wire:model="file_path" />
                    ...
                    <div class="mt-12 grid grid-cols-2 gap-4">
                        <button wire:click.prevent="setPreviousChapter" @class([
                            'flex items-center gap-2 transition-colors',
                            'text-fg-subtext hover:text-fg-title cursor-pointer' =>
                                !(
                                    $activeChapter &&
                                    $activeChapter->id === $masterClass->chapters->first()->id
                                ) && $this->canAccessChapter($activeChapter),
                            'text-fg-subtext/50 cursor-not-allowed' =>
                                $activeChapter &&
                                $activeChapter->id === $masterClass->chapters->first()->id,
                            ...
                            @if (!$this->canAccessChapter($activeChapter)) disabled @endif>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                        ...
                        <button wire:click.prevent="setNextChapter" @class([
                            'flex items-center justify-end gap-2 transition-colors',
                            'text-fg-subtext hover:text-fg-title cursor-pointer' =>
                                $activeChapter->isCompleted() &&
                                $activeChapter->id !== $masterClass->chapters->last()->id &&
                                $this->canAccessChapter($activeChapter),
                            'text-fg-subtext/50 cursor-not-allowed' =>
                                !$activeChapter->isCompleted() ||
                                $activeChapter->id === $masterClass->chapters->last()->id,
                            ...
                            @if (!$activeChapter->isCompleted() || $activeChapter->id === $masterClass->chapters->last()->id) disabled wire:loading.attr="disabled" @endif
                            @if (!$this->canAccessChapter($activeChapter)) disabled @endif>
                            Chapitre Suivant
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
