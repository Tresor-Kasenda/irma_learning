...
Ce guide explique comment créer et gérer les examens pour la plateforme de formation en ligne. Le système d'examen est
déjà entièrement implémenté et fonctionnel.
...
**Pour les examens requis** : Utilisez principalement **Choix unique** et **Choix multiple** comme spécifié dans les
exigences.
...
5. **Ajouter les options de réponse** :
   ...
   **Important** :
    - Pour **Choix unique** : Une seule option peut être marquée comme correcte. Si vous cochez une autre option, la
      précédente sera automatiquement décochée.
...
- option_text
- is_correct (défaut: false)
← CRUCIAL pour marquer les bonnes réponses
- order_position
- image (nullable)
```
